...
Browse to our site <https://gke.nono.io>. You'll see a secure connection icon &
...
We have a Concourse CI server, and we've generated our own keys, but we're
still not secure: people can log in with the user "test" using the password
"test". Yes, really.

We don't want to have any hard-coded users; we want to authenticate against our
GitHub organization, "blabbertabber", so we browse to our organization
(<https://github.com/blabbertabber>) → Settings → Developer Settings → OAuth
Apps → New OAuth App.
...
get the Client ID (`5e4ffee9dfdced62ebe3`) and then click "Generate a new client
